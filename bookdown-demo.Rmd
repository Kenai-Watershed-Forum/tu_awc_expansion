...
fish_dat <- read_sheet(url, sheet = "F_Fish_ID") %>%
  select(-QC1,-QC2,-data_entry,-notes)  %>%
...
z <- fish_dat %>%
  ...
  select(-site_arrive_time, -site_depart_time,-photo_id, -count) %>%
  ...
  mutate(Datum = "",
         ...
         `Name of water body` = site,
         `Observer name (the first and last name of the person handling fish)` = observer_name,
         )
